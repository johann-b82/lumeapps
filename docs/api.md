...
| GET    | /api/hr/embed/birthdays/this-week          | public | Kiosk iframe mirror of the auth'd HR route — schmalere Nutzlast: ohne `birthday` und `age_turning` |
| GET    | /api/hr/embed/joiners/recent               | public | Kiosk iframe mirror of the auth'd HR route |
| GET    | /api/hr/embed/employees/{employee_id}/photo | public | Kiosk iframe mirror of the auth'd HR route — nur für Personen, die gerade auf einem Board stehen (Geburtstag diese Woche oder Eintritt < 52 Wochen); Bytes eine Stunde zwischengespeichert, 60 Abrufe/Minute je IP |
