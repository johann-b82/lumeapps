...
This view measures how many products were inspected — as a **daily rate**. Two metrics per class: **parts per person and day** = inspected quantity ÷ number of (inspector, day) combinations, and **parts per day total** = inspected quantity ÷ number of inspection days. The denominator is built **per class separately**, and "total" gets its own denominator over all rows — so large + small do **not** sum to total. Because the value is a daily rate, week/month/quarter/year only change how many days are averaged; bars stay comparable. The data source is the ERP export **AswQs2151**: one row per inspection booking. On import, each product is automatically classified as **large** or **small** (small includes literature pockets, straps, net variants, life-vest/stowage pouches and all Diehl product groups; everything else is large). Only bookings with cost key "70000" count as a real inspection; tool rows are discarded.

**Article selector:** in the top right you switch between **Finished articles** (default; all articles that do not start with "H"), **Semi-finished articles** (only articles starting with "H", e.g. intermediate inspections) and **All articles**. This lets you hide inspectors who only appear because of booked intermediate inspections (the intermediate inspection is required before the article can be booked onward). The filter applies to the cards, charts and the inspection bookings table.

**KPI cards:** three cards — **Large**, **Small** and **Total** — with the headline *parts/person/day*, the subtitle *parts/day total* and delta badges.

**Charts:** three bar charts — *Large*, *Small* and *Total over time* — with shared granularity and zoom buttons. The bar value is *parts/person/day*; the tooltip also shows parts/day total, parts absolute, person-days and inspection days. A dashed target line appears only when a target is configured (no default anymore — the former 150/400 were unreachable for the corrected daily rate; new targets are set by Quality). Note: "small" is not meaningful at weekly level — the smallest sensible granularity is the month.
