...
import { uploadStockPricesFile } from "@/lib/api";
import type { StockPriceUploadResponse, ValidationErrorDetail } from "@/lib/api";
import { procurementKeys } from "@/lib/queryKeys";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { AdminOnly } from "@/auth/AdminOnly";

interface StockPriceDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function StockPriceDropZone({
  onUploadError,
  onUploadSuccess,
}: StockPriceDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadStockPricesFile,
    onSuccess: (data: StockPriceUploadResponse) => {
      toast.success(t("stock_prices_upload.title"), {
        description: t("stock_prices_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("stock_prices_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("stock_prices_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
