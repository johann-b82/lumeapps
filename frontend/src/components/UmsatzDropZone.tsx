...
import { uploadUmsatzFile } from "@/lib/api";
import type { RevenueUploadResponse, ValidationErrorDetail } from "@/lib/api";
...
/**
 * UmsatzDropZone — admin-only dropzone for AswKpf_RG.txt revenue +
 * credit-note dumps. POSTs to /api/upload-umsatz and invalidates the
 * KPI queries so the Umsatz card + Umsatzwachstum chart refetch.
 */
export function UmsatzDropZone({ onUploadSuccess, onUploadError }: Props = {}) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadUmsatzFile,
    onSuccess: (data: RevenueUploadResponse) => {
      toast.success(t("umsatz_upload.title"), {
        description: t("umsatz_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("umsatz_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("umsatz_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
