...
import { uploadInspectionsFile } from "@/lib/api";
import type {
...
interface InspectionsDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function InspectionsDropZone({
  onUploadError,
  onUploadSuccess,
}: InspectionsDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadInspectionsFile,
    onSuccess: (data: InspectionsUploadResponse) => {
      toast.success(t("inspections_upload.title"), {
        description: t("inspections_upload.summary", {
          inserted: data.rows_inserted,
          replaced: data.rows_replaced,
          large: data.large_count,
          small: data.small_count,
        }),
      });
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("inspections_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("inspections_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
