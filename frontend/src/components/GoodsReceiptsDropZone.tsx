...
import { uploadGoodsReceiptFile } from "@/lib/api";
import type {
...
interface GoodsReceiptsDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function GoodsReceiptsDropZone({
  onUploadError,
  onUploadSuccess,
}: GoodsReceiptsDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadGoodsReceiptFile,
    onSuccess: (data: GoodsReceiptUploadResponse) => {
      toast.success(t("goods_receipts_upload.title"), {
        description: t("goods_receipts_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("goods_receipts_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("goods_receipts_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
