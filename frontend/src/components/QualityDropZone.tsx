...
import { uploadQualityFile } from "@/lib/api";
import type { QualityUploadResponse, ValidationErrorDetail } from "@/lib/api";
...
/**
 * QualityDropZone — admin-only dropzone for 8D (.txt) ingestion.
 * POSTs to /api/upload-quality. Mirrors ContactsDropZone.
 */
interface QualityDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function QualityDropZone({
  onUploadError,
  onUploadSuccess,
}: QualityDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadQualityFile,
    onSuccess: (data: QualityUploadResponse) => {
      toast.success(t("quality_upload.title"), {
        description: t("quality_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("quality_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("quality_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
