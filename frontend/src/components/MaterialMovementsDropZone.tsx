...
import { uploadMaterialMovementsFile } from "@/lib/api";
import type {
...
interface MaterialMovementsDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function MaterialMovementsDropZone({
  onUploadError,
  onUploadSuccess,
}: MaterialMovementsDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadMaterialMovementsFile,
    onSuccess: (data: MaterialMovementsUploadResponse) => {
      toast.success(t("material_movements_upload.title"), {
        description: t("material_movements_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("material_movements_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("material_movements_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
