...
import { uploadDeliveryFile } from "@/lib/api";
import type { DeliveryUploadResponse, ValidationErrorDetail } from "@/lib/api";
import { qualityKeys } from "@/lib/queryKeys";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { AdminOnly } from "@/auth/AdminOnly";

interface DeliveriesDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function DeliveriesDropZone({
  onUploadError,
  onUploadSuccess,
}: DeliveriesDropZoneProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data: DeliveryUploadResponse) => {
      toast.success(t("deliveries_upload.title"), {
        description: t("deliveries_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("deliveries_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("deliveries_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
