...
import { uploadDeliveryReliabilityFile } from "@/lib/api";
import type {
...
interface DeliveryReliabilityDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function DeliveryReliabilityDropZone({
  onUploadError,
  onUploadSuccess,
}: DeliveryReliabilityDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadDeliveryReliabilityFile,
    onSuccess: (data: DeliveryReliabilityUploadResponse) => {
      toast.success(t("otd_upload.title"), {
        description: t("otd_upload.summary", {
