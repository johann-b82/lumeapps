...
import { uploadMaterialPricesFile } from "@/lib/api";
import type {
...
interface MaterialPricesDropZoneProps {
  onUploadError?: (errors: ValidationErrorDetail[]) => void;
  onUploadSuccess?: () => void;
}

export function MaterialPricesDropZone({
  onUploadError,
  onUploadSuccess,
}: MaterialPricesDropZoneProps) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [rejectedExt, setRejectedExt] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: uploadMaterialPricesFile,
    onSuccess: (data: MaterialPricesUploadResponse) => {
      toast.success(t("material_prices_upload.title"), {
        description: t("material_prices_upload.summary", {
        ...
        <div {...getRootProps({ className: containerClass })}>
          ...
          {mutation.isPending ? (
            ...
            <>
              ...
              >
                {t("material_prices_upload.dropzone_prompt")}
              </p>
              ...
              <p className="text-xs text-muted-foreground">
                {t("material_prices_upload.accepted_formats")}
              </p>
            </>
          )}
        </div>
