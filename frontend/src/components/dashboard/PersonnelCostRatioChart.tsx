/**
 * PersonnelCostRatioChart — history bar chart of the Personalkostenquote.
 *
 * Mirrors MaterialCostRatioChart: server owns bucketing via _bucket_windows;
 * this component picks the granularity and renders a configurable target
 * ReferenceLine (from /settings/finance) when one is set.
 */
...
import { fetchPersonnelCostRatioHistory, type BucketGranularity } from "@/lib/api";
import {
  deriveHrBuckets,
  formatMonthYear,
...
export function PersonnelCostRatioChart() {
  ...
  // Configurable target line from /settings/finance (fraction). NULL = hidden.
  const { data: settings } = useSettings();
  const target = settings?.target_personnel_cost_ratio ?? null;
  ...
  const { data, isLoading } = useQuery({
    queryKey: financeKeys.personnelCostRatioHistory(date_from, date_to, granularity),
    queryFn: () =>
      fetchPersonnelCostRatioHistory({ date_from, date_to, granularity }),
  });
  ...
  const chartData = data.map((p) => ({
    month: p.month,
    ratio: p.ratio ?? null,
    personnel_cost: p.personnel_cost,
    revenue: p.revenue,
  }));
    ...
    <Card className="p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <p className="text-sm font-semibold">
          {t("finance.personnelCostRatio.chartTitle")}
        </p>
      ...
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
        <BarChart data={chartData} margin={{ top: 4, right: 8, left: 8, bottom: 4 }}>
          ...
          <Tooltip
            ...
            formatter={(v) =>
              v == null
                ? ["—", t("finance.personnelCostRatio.label")]
                : [formatPercent(Number(v)), t("finance.personnelCostRatio.label")]
            }
          />
