/**
 * ProductionVerzugChart — history bar chart of the Verzugsquote.
 *
 ...
 * and the X-axis label formatter. Bars use the destructive colour — a high
 * Verzug rate is bad (opposite polarity to OTD). No target line.
 */
...
import { fetchProductionVerzugHistory, type BucketGranularity } from "@/lib/api";
import {
  deriveHrBuckets,
  formatMonthYear,
...
export function ProductionVerzugChart() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: productionKeys.verzugHistory(date_from, date_to, granularity),
    queryFn: () =>
  ...
  const chartData = data.map((p) => ({
    month: p.month,
    rate: p.rate ?? null,
    in_verzug_count: p.in_verzug_count,
  ...
  return (
    <Card className="p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <p className="text-sm font-semibold">{t("production.verzug.chartTitle")}</p>
      ...
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
        <BarChart data={chartData} margin={{ top: 4, right: 8, left: 8, bottom: 4 }}>
          ...
          <Tooltip
            ...
            formatter={(v) =>
              v == null
                ? ["—", t("production.verzug.label")]
                : [formatPercent(Number(v)), t("production.verzug.label")]
            }
          />
