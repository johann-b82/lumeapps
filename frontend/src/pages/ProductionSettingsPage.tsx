...
import { ProductionTargetsCard } from "@/components/settings/ProductionTargetsCard";
...
const SCOPE_PATH = "/settings/production";
...
export function ProductionSettingsPage() {
  ...
  const { draft, isDirty, isLoading, isError, isSaving, setField, save, discard } =
    useSettingsDraft({ slice: "production" });
  ...
  return (
    <div
      data-testid="settings-page-production"
    ...
    >
      ...
      {!isLoading && !isError && draft && (
        <>
          <ProductionTargetsCard draft={draft} setField={setField} />
