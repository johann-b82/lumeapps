...
| v1.42 | 2026-05-01 | Drop the Personio binding from the v1.41 sales-activity KPIs. Sales reps are now identified directly by the `Wer` token from the Kontakte file (e.g. `KARRER`, `GUENDEL`) — no aliasing, no Personio sync hook, no sales-department picker. Alembic `v1_42_drop_sales_aliases` drops the `sales_employee_aliases` table and the `app_settings.personio_sales_dept` column. `/api/admin/sales-aliases` and the HR-settings "Sales-rep aliases" section are gone; the contacts-weekly response is now keyed by token (string) instead of personio_employee_id (int); the upload response no longer reports unmapped tokens. The four KPI charts and the orders-distribution card now light up as soon as a Kontakte file has been uploaded — no Personio config required. |
| v1.41 | 2026-05-01 | Sales activity KPIs — four weekly line charts (Erstkontakte, Interessenten, Visits, Angebote) plus an Order-distribution combo card (orders / week / rep, top-3 customer share %, remaining customer share %) on the Sales Dashboard. Driven by a new Kontakte (.txt) upload that populates `sales_contacts` (replace-by-date-range, idempotent) and a new `sales_employee_aliases` table that binds the file's `Wer` token to a Personio employee. Sales reps come from a new `app_settings.personio_sales_dept` config (twin of `personio_production_dept`) on `/settings/hr`; the Personio sync rebuilds canonical aliases each tick (manual aliases survive). Orders/wk/rep is bridged through Kontakte rows whose comment matches `^Angebot <order_number>` since the Aufträge schema doesn't carry sales-rep attribution. Alembic `v1_41_sales_contacts` + new admin endpoint `POST /api/upload-contacts`, viewer endpoints `GET /api/data/sales/{contacts-weekly,orders-distribution}`, and admin alias CRUD at `/api/admin/sales-aliases`. |
| v1.40 | 2026-05-01 | Unify `/signage/media` rhythm with the settings sub-pages — wrap the page body in a single `Card` with a horizontal rule between "add new content" (drop zone + register form) and "added content" (media grid). Drop the per-element dashed-border + `bg-muted` chrome on `MediaUploadDropZone` and `MediaRegisterUrlForm` so they read as plain in-Card sections instead of two parallel cards-within-a-card. Empty-state dropped its own card chrome. `MediaInUseDialog` portals cleanly outside the Card. |
| v1.39 | 2026-05-01 | Per-sensor chart color on `/sensors`, configurable on `/settings/sensors`. Adds nullable `sensors.chart_color VARCHAR(7)` (Alembic `v1_39_sensor_chart_color`); SensorRead/Create/Update validate `^#[0-9a-fA-F]{6}$`. The settings row form gains a fourth column on the scales row — native `<input type="color">` plus a "use default" button to clear back to the palette. The time-series chart prefers `sensor.chart_color` and falls back to the existing palette index when NULL, so the same physical device keeps its color when other sensors are added or removed. |
| v1.38 | 2026-05-01 | Solid sensor chart lines on `/sensors` — flip both temperature and humidity `<Line>` components from `connectNulls={false}` to `connectNulls={true}` so every sensor reads as one continuous line that bridges NULL poll cycles, instead of a noisy string of disconnected dots. Per-sensor color is configurable in v1.39. |
| v1.37 | 2026-05-01 | `/signage/media` polish — "Dateien auswählen" now uses the same primary blue as "URL registrieren" (drop the outline variant on the drop zone's browse button). The two text inputs in the inline register form get `bg-background` so they read as fields against the surrounding muted form card instead of blending in. SettingsSectionPicker test hardened with `findByRole` to wait for base-ui's Select portal to actually mount the listbox content (was a pre-existing flake in the full vitest suite). |
| v1.36 | 2026-05-01 | Inline the URL/HTML register form on `/signage/media`. The "Externen Inhalt registrieren" content moved out of the modal dialog and into the page directly as the right half of the top-of-page grid (drop zone on the left, register form on the right). The form is self-contained — submit creates the `signage_media` row via the existing Directus path, then resets so the next entry can be typed without extra interaction. `MediaRegisterUrlDialog.tsx` was renamed to `MediaRegisterUrlForm.tsx` (git mv preserves blame); the `<Dialog>` wrapper, `open` / `onOpenChange` props, the `registerOpen` state on `MediaPage`, and the previous click-to-open CTA card all went away. |
| v1.35 | 2026-05-01 | Side-by-side drop zone + register-URL CTA on `/signage/media`. The upload drop zone and the "URL / HTML registrieren" trigger now stand at the top of the page in a `grid grid-cols-1 md:grid-cols-2` so both read as equal entry points (tablet+ side-by-side, mobile stacked). The new register CTA mirrors the drop zone's dashed-border styling and opens the existing `MediaRegisterUrlDialog`. The previous bottom-right small outline button is gone. |
| v1.34 | 2026-05-01 | Drop top-level page headers from all Digital Signage pages — same v1.29 pattern as Settings. The SubHeader dropdown on `/signage/*` already shows the active section (Media / Playlists / Devices / Schedules), so the per-page `<SectionHeader title=… description=…>` block at the top of each was redundant. `MediaPage`, `PlaylistsPage`, `DevicesPage`, and `SchedulesPage` (across loading / error / default render branches) drop their SectionHeader plus the now-unused import. `PairPage` drops the title + subtitle `<div>` at the top of the pairing card; the NavBar breadcrumb continues to identify the page. Inner card content (empty-state titles, PlaylistEditor's "Vorschau" / "Inhalte" subsection labels, table column headers) stays intact — content-internal, not page chrome. |
| v1.33 | 2026-05-01 | Equal vertical gaps on settings sub-pages — `space-y-8` → `space-y-4` on the wrapper of `/settings/general`, `/settings/hr`, and `/settings/sensors` so the inter-Card gap (16 px) matches the SubHeader-to-first-Card gap (`pt-4` = 16 px). Visible on Sensoren where Sensoren → SNMP-Walk → Abfragetakt → Globale Schwellenwerte now step at one consistent rhythm. |
| v1.32 | 2026-05-01 | Unify settings page rhythm with Sensors — `/settings/hr` now stacks two top-level Cards (Personio + Sollwerte) instead of one Card with embedded subsections separated by an `<hr>`, matching how `/settings/sensors` stacks four Cards. Wrapper rhythm on Allgemein and HR reverted to `pt-4 space-y-8` so all three settings sub-pages share one vertical pattern. Dropped the `mt-6` left over on `PersonioCard` / `HrTargetsCard` non-embedded outer Cards — the wrapper's `space-y-8` now owns the gap. |
| v1.31 | 2026-05-01 | Settings typography + rhythm — embedded subsection headings (Personio, Sollwerte) bumped from `text-base` to `text-xl` so they match the Darstellung CardTitle on `/settings/general`. Top padding above the first section reduced (`pt-4` → `pt-2`), inter-section gap tightened (`space-y-8` → `space-y-4`/`-6`), and the v1.30 `pt-6` compensation on the HR Card body dropped now that the subsection h3 carries the visual weight. |
| v1.30 | 2026-05-01 | HR settings polish — drop the outer `Card` titled "HR" on `/settings/hr`. The SubHeader dropdown already shows "HR ▾" in its trigger; nesting Personio + Sollwerte inside a Card with the same title was redundant. Personio and Sollwerte stand on their own inside a plain Card body, separated by an `<hr>` as before. |
| v1.29 | 2026-05-01 | Settings polish — drop the redundant section `<h1>` on `/settings/general`, `/settings/hr`, and `/settings/sensors`. The SubHeader dropdown introduced in v1.28 already shows the active section in its trigger ("Allgemein ▾" / "HR ▾" / "Sensoren ▾"); rendering the same label as a page header below it doubled up. SensorsSettingsPage's `<SectionHeader>` block went the same way along with its now-unused import. |
...
| v1.25 | 2026-04-30 | Mobile UI Hygiene Pass — six dashboard/admin surfaces (`DateRangeFilter`, `SubHeader` Signage tabs, `EmployeeTable` filter pill, `SalesTable` + `EmployeeTable` bodies, `SettingsPage` color grid, `NavBar`) made usable at 375 px. Pill→`<Select>` swap below `md:` (768 px); tables get a `min-w` so columns don't compress inside their existing `overflow-x-auto` wrapper; `SettingsPage` drops the crowded `md:grid-cols-3` middle step; `NavBar` hides the breadcrumb on mobile and moves Theme + Language toggles into the `UserMenu` dropdown. Tailwind dual-render strategy — same component renders both layouts, one is hidden via `hidden md:block` / `md:hidden`; both branches are mounted but only one is visible at the active breakpoint. Six new vitest tests cover the DOM contract per swap. Surfaced two pre-existing test-infra gaps and fixed them: jsdom shims for `ResizeObserver` + `matchMedia` (broke `toggle.test.tsx` since the initial commit), and a `tests/e2e/**` exclude in `vitest.config.ts` so Playwright specs don't get auto-discovered. No backend changes. |
